...
\begin{itemize}
    ...
    \item ELSE IF $s$ IS eligible for $r$ and ONLY applied to $(r,c)$; either $s$ applied to $(r,c)$, was accepted and was later rejected from $r$ (and therefore also $c$ since $r$ only serves $c$) in favour of a higher priority routed student, OR $r$ was already at capacity at the point of application and $s$ was rejected as the lowest priority routed student. also add college and later students.
...
A key aspect of stability is the order in which the oversubscription is resolved: in the case where both college and route are oversubscribed, as long as route oversubscription is resolved first, college oversubscription is resolved simultaneously. Additionally, matchings may be stable, even if routes are undersubscribed.
...
\begin{enumerate}
    ...
    \item $(r_1,s_4)$ applies and now there is a problem: both $r_1$ and $c_1$ are at capacity, and the order in which the oversubscription is resolved is important:
    \begin{itemize}
        \item If $c_1$'s oversubscription is resolved first, then $s_3$ will be rejected from the college, after which $(r_1,s_6)$ will be rejected from both route and college, leaving the college \textit{undersubscribed}, but importantly violating one of the principles that guarantees termination and stability: that colleges only "trade up."
        \item On the contrary, if the route oversubscription is resolved first, then only $(r_1,s_6)$ will be rejected from both route and college, which satisfies both capacity conditions simultaneously.
    \end{itemize}
\end{enumerate}
...
After following the same procedure as above, $s_1,s_2$ now apply to $c_1$ in reverse order as before, displacing first $s_3$, then $(r_1,s_5)$. It is obvious to see that this will leave $r_1$ \textit{undersubscribed}. This is not an issue here however because there are no students, routed or otherwise who would be able to that the college prefers, as all other acceptable students have already been rejected.
