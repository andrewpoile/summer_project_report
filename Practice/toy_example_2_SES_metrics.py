...
class Student:
    def __init__(self,name,preferences,routes=None,locals=None):
        self.name = name
...
def initialise(
        ...
        max_students = 20, max_colleges = 5, max_college_capacity = 10, max_route_capacity = 10,
    ...
    n_students = randint(1,max_students) if randomised else n_students
    n_colleges = randint(1,max_colleges) if randomised else n_colleges
    ...
    c_caps = {c:randint(1,max_college_capacity) for c in college_names} if randomised else {c:c_cap for c in college_names}
    r_caps = {r:randint(1,max_route_capacity) for r in route_names} if randomised else {r:r_cap for r in route_names}
...
def college_oversubscription(free,matching,s,c,r,verbose=False):
...
def route_oversubscription(free,matching,s,c,r,verbose=False):
    """Run if current route capacity reaches 0."""
    if verbose: print(f"{r} has reached capacity!")
    if verbose: print(f"{r} serves {c}")
    if verbose: print(f"current priority list for {c}: {colleges[c].priorities}")
...
def greedy_matching(students, colleges, routes, verbose=False):
    ...
    while free:
        ...
        s = free.pop(0) #pop the first student in the list
        p = students[s].preferences.pop(0) if students[s].preferences else p is None #pop the top college on s's preference list and handle empty preference lists
        if not p: unassigned.append(s) ;continue #if s's preference list is empty, move it to unassigned
    ...
    return matching, unassigned, di

def routed_acceptance(
        student_names, college_names,
        students, colleges, routes,
        verbose=False
        ):
    ...
    while free:
        ...
        print(f"assigning student {s}", f"{s} has preferences:\n {students[s].preferences}", sep="\n") if verbose else None
        ...
        colleges[extract_college(p)].assigned_students.append(match_to_priority(s,p))
        ...
        if p in college_names and type(p) is not tuple:
            c = p ;r = None #;print(f"college, preference: {c}, {p}")
            
        elif any(item in p for item in college_names):
            r,c = p[0],p[1] #;print(f"route, college, preference: {r}, {c}, {p}")
            if routes[r].capacity == 0: route_oversubscription(free,matching,s,c,r) ;continue
            else: routes[r].capacity -= 1

        if colleges[c].capacity == 0: college_oversubscription(free,matching,s,c,r)
        else: colleges[c].capacity -= 1
        
        if verbose: print(f"current matching is:\n {matching}")
        if unassigned and verbose: print(f"currently unassigned students:\n {unassigned}")
        #break
...
for i in range(1000):
    ...
    student_names, college_names, route_names, student_preferences, college_priorities, students, colleges, routes = initialise()

    greed_matching, greed_unassigned, greed_di = greedy_matching(students, colleges, routes)

    route_matching, route_unassigned, route_di = routed_acceptance(student_names, college_names,
                                                                   students, colleges, routes)

    greed_blocking_pairs = check_stability(student_preferences, college_priorities,
                                     colleges, routes,
                                     greed_matching, greed_unassigned,
                                     verbose=False)
    
    route_blocking_pairs = check_stability(student_preferences, college_priorities,
                                     colleges, routes,
                                     route_matching, route_unassigned,
                                     verbose=False)

    print(greed_di,greed_blocking_pairs,100*"-",
          route_di,route_blocking_pairs,100*"-",sep="\n")